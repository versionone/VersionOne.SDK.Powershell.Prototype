...
[CmdletBinding(DefaultParameterSetName = 'ScriptInputSet')]
    ...
    End
    {
        function getAttribute([string] $line )
        {

        }

        [System.IO.File]::AppendAllText("c:\temp\tabexpansion2.txt", "SetName: $($psCmdlet.ParameterSetName) Options: $options InputScript: '$inputScript'`n")
        ...
        $options["CustomArgumentCompleters"] = @{
            "assetType" = { "Epic","EpicCategory", "Somethingelse" }
            "attributes" = { getAttribute($inputScript) }
        }
